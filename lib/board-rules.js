...
// another roll. Landing on an opponent's ring cell captures it back to base;
// the two start cells (abs 0 and 26) are safe. Exact roll needed to finish.
const LUDO_START = { 1: 0, 2: 26 };
const LUDO_SAFE = new Set([0, 26]);
const LUDO_HOME = 57;

function ludoTokens(state, player) { return player === 1 ? state.p1 : state.p2; }
  ...
  applyMove(state, player, move) {
    ...
    if (type === 'roll') {
      ...
      if (legal.length === 0) {
        ...
        return {
          state: { ...state, die, phase: 'roll', currentPlayer: opp, lastEvent: 'no-move' },
          gameOver: false, winner: null,
        };
      }
      return {
        state: { ...state, die, phase: 'move', lastEvent: null },
        gameOver: false, winner: null,
      };
    }
    ...
    if (type === 'move') {
      ...
      const theirs = ludoTokens(state, opp).slice();
      const pos = mine[token];
      const newPos = pos === -1 ? 0 : pos + die;
      mine[token] = newPos;
      let lastEvent = null;

      // Capture on the shared ring (home column is private, start cells safe).
      if (newPos <= 50) {
        const abs = (LUDO_START[player] + newPos) % 52;
        if (!LUDO_SAFE.has(abs)) {
          for (let i = 0; i < 4; i++) {
            const tp = theirs[i];
            if (tp >= 0 && tp <= 50 && (LUDO_START[opp] + tp) % 52 === abs) {
              theirs[i] = -1;
              lastEvent = 'capture';
            }
          }
      ...
      const next = {
        ...state,
        p1: player === 1 ? mine : theirs,
        p2: player === 2 ? mine : theirs,
        die,
        lastEvent,
      };
      if (mine.every(t => t === LUDO_HOME)) {
        ...
        return { state: next, gameOver: true, winner: String(player) };
      }
      // A six earns another roll; otherwise the turn passes.
      next.phase = 'roll';
      next.currentPlayer = die === 6 ? player : opp;
      return { state: next, gameOver: false, winner: null };
    }
